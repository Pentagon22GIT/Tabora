...
- destination layoutのAX mutationはPIDごとのlaneで実行します。同一アプリの複数windowは直列、異なるアプリは並列とし、各windowは既存の有限`setFrameReliably`をそのまま使用します。これはAX書き込み順の競合を避ける実行順序であり、layout plannerや2 / 3 / 4 group geometryを変更しません。
