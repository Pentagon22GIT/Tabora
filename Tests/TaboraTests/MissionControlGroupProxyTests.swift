...
final class MissionControlGroupProxyTests: XCTestCase {
    func testGeometryConfirmationSurvivesPositionOnlyChanges() {
        let original = geometryConfirmationKey()
        var candidate = MissionControlPreviewGeometryConfirmationCandidate(
            key: original, observationCount: 1, firstObservedAt: 10
        )
        let translated = geometryConfirmationKey(x: 200, y: -80)
        XCTAssertNotEqual(original, translated)
        XCTAssertEqual(candidate.advance(matching: translated, now: 10.05), false)
        XCTAssertEqual(candidate.advance(
            matching: geometryConfirmationKey(x: 300, y: -40), now: 10.2
        ), true)
        XCTAssertEqual(candidate.firstObservedAt, 10)
    }

    func testGeometryConfirmationRejectsChangedSizeDisplayOrPhysicalWindow() {
        let original = geometryConfirmationKey()
        for changed in [
            geometryConfirmationKey(width: 601),
            geometryConfirmationKey(height: 401),
            geometryConfirmationKey(displayID: 2),
            geometryConfirmationKey(pid: 200),
            geometryConfirmationKey(windowID: 11),
            geometryConfirmationKey(identity: "other")
        ] {
            var candidate = MissionControlPreviewGeometryConfirmationCandidate(
                key: original, observationCount: 1, firstObservedAt: 10
            )
            XCTAssertNil(candidate.advance(matching: changed, now: 10.2))
            XCTAssertEqual(candidate.observationCount, 1)
        }
    }

    private func geometryConfirmationKey(
        x: Int = 0, y: Int = 0, width: Int = 600, height: Int = 400,
        displayID: CGDirectDisplayID = 1, pid: pid_t = 100,
        windowID: CGWindowID = 10, identity: String = "member"
    ) -> MissionControlPreviewCacheKey {
        MissionControlPreviewCacheKey(
            displayID: displayID, pid: pid, windowID: windowID,
            stableIdentity: identity, frameMinX: x, frameMinY: y,
            frameWidth: width, frameHeight: height
        )
    }

    func testRetiredSelectionCallbackCannotCancelNewCandidateOnSameProxy() {
        let checked = expectation(description: "Old selection settlement ran")
        DispatchQueue.main.async {
            _ = NSApplication.shared
            let proxy = MissionControlGroupProxyWindow()
            proxy.presentedMemberIDs = ["a", "b"]
            proxy.currentTransitionAuthorization = { _ in true }
            var generation: UInt64 = 0
            var released: [UInt64] = []
            var consumed: [UInt64] = []
            proxy.beginSelectionConfirmation = { _, _ in
                generation += 1
                return generation
            }
            proxy.releaseSelectionConfirmation = { candidate, _, _ in
                released.append(candidate)
            }
            proxy.consumeSelectionConfirmation = { candidate, _, _ in
                consumed.append(candidate)
                return .queuedMigrationForegroundIntent
            }
            let notification = Notification(
                name: NSWindow.didBecomeKeyNotification, object: proxy
            )
            proxy.windowDidBecomeKey(notification)
            proxy.cancelSelectionTransition()
            proxy.windowDidBecomeKey(notification)
            XCTAssertEqual(generation, 2)
            // Both first observations run at 0.14s. A current candidate whose
            // application is unsettled retries no earlier than another 0.06s.
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.18) {
                XCTAssertTrue(released.isEmpty)
                XCTAssertTrue(
                    proxy.isSelectionConfirmationPending || consumed == [2]
                )
                proxy.retire()
                checked.fulfill()
            }
